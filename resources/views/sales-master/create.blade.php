<x-app-layout>
    <div class="p-6 text-gray-900 antialiased bg-white min-h-screen text-sm">
        ...
        {{-- 新規登録フォーム --}}
        <form method="POST" action="{{ route('sales-master.store') }}" onsubmit="return confirm('販売マスタを登録してよろしいですか？');" class="max-w-5xl">
            @csrf
                ...
                <table class="w-full text-left table-fixed border-collapse">
                    <tbody>
                        <tr class="border-b border-gray-200">
                            ...
                            <td class="w-3/4 p-4">
                                <input type="text" name="product_name" value="{{ old('product_name') }}" required class="w-full border border-gray-300 rounded px-3 py-1.5 focus:border-orange-500 focus:ring-1 focus:ring-orange-500">
                            </td>
                        </tr>
                        <tr class="border-b border-gray-200">
                            <th class="bg-gray-50 p-4 font-medium text-gray-700 align-middle border-r border-gray-200">Amazon販売ページURL</th>
                            <td class="p-4">
                                <input type="url" name="amazon_url" value="{{ old('amazon_url') }}" class="w-full border border-gray-300 rounded px-3 py-1.5 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 font-mono text-xs">
                            </td>
                        </tr>
                        <tr class="border-b border-gray-200">
                            <th class="bg-gray-50 p-4 font-medium text-gray-700 align-middle border-r border-gray-200">TikTok販売ページURL</th>
                            <td class="p-4">
                                <input type="url" name="tiktok_url" value="{{ old('tiktok_url') }}" class="w-full border border-gray-300 rounded px-3 py-1.5 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 font-mono text-xs">
                            </td>
                        </tr>
                        <tr>
                            <th class="bg-gray-50 p-4 font-medium text-gray-700 align-middle border-r border-gray-200">ドライブパス</th>
                            <td class="p-4">
                                <input type="text" name="drive_path" value="{{ old('drive_path') }}" class="w-full border border-gray-300 rounded px-3 py-1.5 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 font-mono text-xs">
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div id="variations-container" data-initial-variations="{{ json_encode(old('variations', [])) }}" class="space-y-6 mb-8"></div>
