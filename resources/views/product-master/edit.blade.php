<x-app-layout>
    <div class="p-6 text-gray-900 antialiased bg-white min-h-screen text-sm">
        ...
        <input type="hidden" id="saved-selling-places" value="{{ json_encode(old('selling_places', $product->selling_places ?? [])) }}">

        <form method="POST" action="{{ route('product-master.update', $product->seq) }}" enctype="multipart/form-data" onsubmit="return confirm('商品の登録内容を更新しますか？');">
            @csrf
            @method('PUT')
                ...
                <table class="w-full text-left table-fixed border-collapse">
                    <tbody>
                        ...
                        <tr class="border-b border-gray-200">
                            <th class="w-1/4 bg-gray-50 p-3 font-medium text-gray-700 align-top border-r border-gray-200 pt-4">商品画像</th>
                            <td class="w-3/4 p-3 align-middle">
                                @if($product->image_path)
                                    <div class="mb-2">
                                        ...
                                        <p class="text-xs text-gray-400 mt-1">現在の画像</p>
                                    </div>
                                @endif
                        ...
                        <tr class="border-b border-gray-200">
                            ...
                            <td class="p-3 align-middle">
                                <input type="text" name="product_management_code" value="{{ old('product_management_code', $product->product_management_code) }}" class="w-full max-w-sm border border-gray-300 rounded px-3 py-1.5 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-sm font-mono">
                            </td>
                        </tr>
                        <!-- 仕入先商品名 -->
                        <tr class="border-b border-gray-200">
                            ...
                            <td class="p-3 align-middle">
                                <input type="text" name="supplier_product_name" value="{{ old('supplier_product_name', $product->supplier_product_name) }}" class="w-full border border-gray-300 rounded px-3 py-1.5 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-sm">
                            </td>
                        </tr>
                        <!-- 仕入れ値 -->
                        <tr class="border-b border-gray-200">
                            ...
                            <td class="p-3 align-middle">
                                <div class="flex items-center">
                                    ...
                                    <input type="number" name="buying_price" value="{{ old('buying_price', $product->buying_price) }}" class="w-full max-w-xs border border-gray-300 rounded px-3 py-1.5 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-sm font-mono text-right">
                                </div>
                            </td>
                        </tr>
                        <!-- ドライブパス -->
                        <tr class="border-b border-gray-200">
                            ...
                            <td class="p-3 align-middle">
                                <input type="url" name="drive_path" value="{{ old('drive_path', $product->drive_path) }}" class="w-full border border-gray-300 rounded px-3 py-1.5 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-sm font-mono">
                            </td>
                        </tr>
                        <!-- 販売先（動的生成＆活性化） -->
                        <tr>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('product-master.show', $product->seq) }}" class="bg-slate-300 hover:bg-slate-400 text-slate-700 font-bold py-2 px-10 rounded shadow-sm transition duration-150 tracking-wider">
                    キャンセル
                </a>
            </div>
        </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const savedSellingPlaces = JSON.parse(document.getElementById('saved-selling-places').value || '[]');

            const selectEl = document.getElementById('management_code');
            const container = document.getElementById('selling-places-container');

            // 一意の初期フラグ。最初の一回だけエラーメッセージが出ていない場合にチェックボックスを生成。
            function updateCheckboxes() {
                ...
                allowedPlaces.forEach(market => {
                    ...
                    if (savedSellingPlaces.includes(market)) {
